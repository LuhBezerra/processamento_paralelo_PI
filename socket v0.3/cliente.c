...
//para compilar: gcc cliente.c -o cliente -lm
...
float interativo()
{
    ...
    float dividendo = pow(-1, parcela);
    float divisor = (2 * parcela) +1;

    float resultado = dividendo/divisor;
}

float lote() // esse 'inicio' e 'final' seram os valores recebidos pelo servidor(min,max)
{
    float soma ,dividendo, divisor, resultado;
