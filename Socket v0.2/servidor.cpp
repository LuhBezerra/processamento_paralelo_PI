...
void funcionalidades(int x = 0,int y = 0); // precisa criar ela aqui em cima tbm para nao dar erro no escopo, mais necessariamente essa mensagem -> was not declared in this scope
...
int main (int argc, char * argv [])
{
    ...
    while (1) {
        ...
        if (novoProcesso == 0)  {
            close(sockFd);
            funcionalidades(novoSockFd,modo);
            exit(0);
        }
...
void funcionalidades (int sock, int modoExecucao) // So chega nessa parte se um cliente conseguir conectar-se com êxito ao servidor
{   
    char buffer[1024];
    int saida, x , modo, enviaModo;

    modo = modoExecucao;
    if (modo == 1) enviaModo = write(sock,"Será no modo interativo",25);
    if (modo == 2) enviaModo = write(sock,"Será no modo lote",25);
